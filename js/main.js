...
d3.csv("numThrows.csv", rowConverter, function(data) {
  ...
  svg.append("text")
     ...
     .text("The number of throws that were attempted in each game for all three divisions. \
     Scroll over each circle for game details.")
     .style("font-size", 12)
     .style("font-family", "sans-serif")
     .call(wrap, w-50);
  ...
  d3.selectAll("#input")
    .on("change", function() {
      ...
      if (selection==2016 | selection==2017 | selection=="within" | selection=="over") {
        ...
        highlight.style("fill", "none"); // remove highlight text
